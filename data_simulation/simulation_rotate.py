...
class RotatingRodSimulator:
    def __init__(self,
                 omega=1.0,        # 初始角速度 rad/s
                 alpha=-0.05,      # 角加速度 (负数表示逐渐减速)
                 l0=3.0,           # 初始杆长
                 dl=1.0,           # 杆长增长速度
                 theta0=0.0,       # 初始角度
                 center0=(9, 6),   # 初始中心位置
                 ...
                 linewidth=24):
        self.omega0 = omega   # 初始角速度
        self.alpha = alpha    # 角加速度
        self.l0 = l0
        self.dl = dl
        self.theta0 = theta0
    ...
    def _compute_omega(self, t):
        """角速度随时间演化"""
        return max(0.0, self.omega0 + self.alpha * t)  # 不允许负数
...
if __name__ == "__main__":
    ...
    base_output_dir = "unified_simulation_val/rotate"
    os.makedirs(base_output_dir, exist_ok=True)

    n_samples = 10  # 可以改成 100
