...
class RotatingRodSimulator:
    ...
    def render_frame(self, t):
        ...
        ax.plot([p0[0], p1[0]], [p0[1], p1[1]], color="red", linewidth=self.linewidth)
...
if __name__ == "__main__":
    ...
    for i in range(n_samples):
        omega = float(np.random.uniform(1.5, 2.0))
        l0 = float(np.random.uniform(2.0, 3.0))
        dl = 0.1
        theta0 = math.pi / 4
        x0 = np.random.uniform(10, 12)
        y0 = np.random.uniform(8, 9)
        vx = np.random.uniform(-4.0, -3.0)
