...
from models.nnd_motion_rotate_fall import NewtonODELatent  # nnd.py 文件路径
...
BATCH_SIZE = 64
SAVE_EVERY = 1000
OUT_DIR = Path("runs_nnd_rotate_fall")
OUT_DIR.mkdir(parents=True, exist_ok=True)

# ---------------- Load motion data ----------------
motion_batch = torch.load(
    "/home/yuan418/data/project/Newtongen_ICLR/physicsl_label/physical_rotate_fall.pt"
)  # shape (B_total,T,C)
B_total, T, C = motion_batch.shape
print(f"Loaded motion_batch.pt: shape {motion_batch.shape}")

#[x,y,vx,vy,theta,omega,s,l]
motion_batch = motion_batch[:, :, :8]
z0_data = motion_batch[:, 0, :]  # 初始状态 (B_total, 8)
...
for epoch in range(1, EPOCHS + 1):
    ...
    for i in range(0, B_total, BATCH_SIZE):
        ...
        z0_batch = z0_data[idx]              # (batch_size,8)
        motion_batch_b = motion_batch[idx]   # (batch_size,T,8)

        # Forward
        motion_pred = model(z0_batch, ts)    # (batch_size, T, 8)
...
# ---------------- Save final model ----------------
final_path = OUT_DIR / "newton_ode_rotate_fall_final.pth"
