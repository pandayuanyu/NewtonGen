...
MODEL_PATH = Path("/home/yuan418/data/project/Newtongen_ICLR/runs/parabola/learnedODE_parabola_epoch18000.pth")

# 这里可以定义多个配置，每个dict对应一组z0/DT/METER_PER_PX/chosen_shape
config_list = [
    dict(
        z0=[2.0, 10.0, 5.5, -1.2, 0.0, 0.0, 0.6, 0.8, 0.32],
        DT=0.02,
        METER_PER_PX=0.05,
        chosen_shape="circle",
        output_name="set_a"
...
for cfg in config_list:
    ...
    out_dir = Path(f"inference/parabola/{cfg['output_name']}")
    ...
    noisewarp_out = nw.get_noise_from_video(
        ...
        save_files=True,
        noise_channels=16,
        output_folder=f"inference/parabola/NoiseWarp_{cfg['output_name']}",
        resize_frames=1,
        resize_flow=1,
        downscale_factor=4,
...
if __name__ == "__main__":
    ...
    for cfg in config_list:
        ...
        sample_path = f"inference/parabola/NoiseWarp_{cfg['output_name']}" 

        for i, prompt in enumerate(prompt_list):
            # 每个 prompt 生成的输出 MP4 文件名
            output_mp4_path = f"inference/parabola/{cfg['output_name']}_prompt{i+1}.mp4"
