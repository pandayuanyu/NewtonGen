...
def get_noise_from_video(
    ...
    warp_kwargs=dict(),
    external_flows_path: str = None,  # new added
):
    """
    Extract noise from a video using either external flows or RAFT.
    """
    ...
    # Load external flows if provided
    if external_flows_path is not None:
        numpy_flows_array = np.load(external_flows_path)  # shape [T-1, 2, H, W]
        assert numpy_flows_array.ndim == 4 and numpy_flows_array.shape[1] == 2, \
            "External flows must have shape [T-1, 2, H, W]"
    else:
        raft_model = raft.RaftOpticalFlow(device, "large")
        numpy_flows_array = None
    ...
    if resize_frames is not None:
        rp.fansi_print(f"Resizing all input frames to size {resize_frames}", 'yellow')
    ...
    # Convert frames to float32 [0,1]
    video_frames = rp.as_rgb_images(video_frames)
    video_frames = np.stack(video_frames).astype(np.float16) / 255
    ...
    if h % downscale_factor or w % downscale_factor:
        rp.fansi_print(
            f"WARNING: height {h} or width {w} is not divisible by downscale_factor {downscale_factor}.",
            'yellow'
        )
    ...
    try:
        for index, video_frame in enumerate(tqdm(video_frames[1:])):
            # Use external flow if provided
            if numpy_flows_array is not None:
                dx, dy = [torch.tensor(f, device=device, dtype=torch.float32) for f in numpy_flows_array[index]]
            else:
            ...
            if visualize:
                ...
                optional_images, optional_labels = [], []
                if remove_background:
                    alpha = alphas[index]
                ...
                visualization = rp.as_byte_image(
                    rp.tiled_images(
                        rp.labeled_images(
                            [down_noise_image / 3 + 0.5, down_video_frame, down_flow_rgb, down_noise_image / 5 + down_video_frame]
                            + optional_images,
                            ["Warped Noise", "Input Video", "Optical Flow", "Overlaid"] + optional_labels,
                            font='G:Zilla Slab'
                        )
                    )
                )
        ...
        rp.fansi_print(f"Interrupted! Returning {len(numpy_noises)} noises", "cyan", "bold")
    ...
    if save_files:
        ...
        if "ffmpeg" in rp.get_system_commands():
            ...
            rp.save_video_mp4((numpy_noises / 4 + 0.5)[:, :, :, :3], noise_mp4_path, video_bitrate="max", framerate=30)
        ...
        rp.fansi_print(f"Saved noises to {noises_path} and flows to {flows_path}", "green")
