...
import os
os.environ['TRANSFORMERS_CACHE'] = "/home/yuan418/data/project/sam2/weightss/"
os.environ["TORCH_HOME"] = "/home/yuan418/data/project/sam2/weightss/"
...
class PhysicalEncoder:
    ...
    def extract_physical_features(self, video_masks):
        ...
        x, y = centers_m[:, 0], centers_m[:, 1]
        theta = np.arctan2(x - 9, 12 - y) 
        omega = np.zeros_like(theta)
        omega[1:-1] = (theta[2:] - theta[:-2]) / (2 * self.delta_t)
...
if __name__ == "__main__":
    video_dir = "/home/yuan418/data/project/Newtongen_ICLR/physicsl_label/unified/circle"
    out_path = "physical_circle.pt"

    encoder = PhysicalEncoder(save_video=True, mask_outdir="mask_videos_circle",
    ...
    for fname in sorted(os.listdir(video_dir)):
        if fname.lower().endswith(".mp4"):
            ...
            print(f"[INFO] Extracted physical labels (前 {num_show} 帧):")
            print("格式: x, y, vx, vy, theta(rad), omega(rad/s), s(short), l(long), a(area)")
