...
BATCH_SIZE = 64
SAVE_EVERY = 1000
OUT_DIR = Path("runs/circle")
OUT_DIR.mkdir(parents=True, exist_ok=True)

# ---------------- Load latent dynamics data ----------------
dynamics_batch = torch.load(
    "/home/yuan418/data/project/Newtongen_ICLR/physicsl_label/unified/labels/physical_circle.pt"
)  # shape (B_total,T,C)
...
weights = torch.tensor([0.05, 0.05, 0.05, 0.05,   # (x,y,vx,vy)
    ...
    if epoch % SAVE_EVERY == 0 or epoch == 1:
        ...
        with torch.no_grad():
            dynamics_batch_val = torch.load(
                "/home/yuan418/data/project/Newtongen_ICLR/physicsl_label/unified/labels_val/physical_gt_circle.pt"
            )
        ...
        # ---------------- Save checkpoint ----------------
        ckpt_path = OUT_DIR / f"learnedODE_circle_epoch{epoch:05d}.pth"
        torch.save(model.state_dict(), ckpt_path)
        print(f"Checkpoint saved: {ckpt_path}")

# ---------------- Save final model ----------------
final_path = OUT_DIR / "learnedODE_circle_final.pth"
